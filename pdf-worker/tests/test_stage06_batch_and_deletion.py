...
def test_batch_upload_reuses_same_filename_qr_and_creates_new_names(admin_client):
    ...
    existing = admin_client.post(
        ...
        files={"file": ("答案.pdf", pdf_bytes(), "application/pdf")},
        follow_redirects=False,
    )
    assert existing.status_code == 303
    existing_qr = existing.headers["location"].split("/")[3].split("?")[0]
    ...
    response = admin_client.post(
        ...
        files=[
            ...
            ("files", ("新答案.pdf", pdf_bytes(2), "application/pdf")),
        ],
        follow_redirects=False,
    )
    ...
    assert [item["resolved_title"] for item in batch["items"]] == ["答案", "新答案"]
    assert batch["items"][0]["qr_id"] == existing_qr
    assert all(item["status"] == "completed" for item in batch["items"])
    listing = admin_client.get("/admin/materials", params={"q": "新答案", "page_size": 100})
    assert listing.status_code == 200
    assert "新答案" in listing.text
...
def test_batch_partial_failure_does_not_leave_failed_resource(admin_client):
    ...
    with admin_client.app.state.database.read() as connection:
        assert connection.execute("SELECT COUNT(*) FROM answer_resources").fetchone()[0] == 2
        assert connection.execute(
            "SELECT COUNT(*) FROM batch_import_items WHERE status = 'failed' AND resource_id IS NOT NULL"
        ).fetchone()[0] == 1


def test_batch_rejects_duplicate_filenames_in_same_selection(admin_client):
    csrf = csrf_from(admin_client.get("/admin/materials/import"))
    response = admin_client.post(
    ...
    assert response.json()["error"]["code"] == "DUPLICATE_BATCH_NAME"
