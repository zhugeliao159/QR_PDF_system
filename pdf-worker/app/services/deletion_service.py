...
class PermanentDeletionService:
    ...
    def delete_one(self, qr_id: str, actor: str) -> dict[str, Any]:
        with self.database.read() as connection:
            ...
            qr_storage_keys = [
                f"qr-codes/{row['public_token']}.png"
                for row in connection.execute(
                    "SELECT public_token FROM qr_aliases WHERE resource_id = ?",
                    (resource["id"],),
                ).fetchall()
            ]
        ...
        try:
            ...
            for storage_key in qr_storage_keys:
                path = self.storage.resolve(storage_key, must_exist=False)
                if path.is_file():
                    moved_files.append(
                        (self.storage.move_to_trash(storage_key), storage_key)
                    )
