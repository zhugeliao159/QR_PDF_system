...
当前仍是 `192.168.1.20:18081` 的机构 LAN HTTP 测试模式，未修改防火墙、路由、Tailscale 或 DNS，也未执行公网部署。正式公网前必须完成稳定域名与 HTTPS、可信反向代理、学生/管理员入口隔离、Cookie Secure、告警监控、异机加密备份与恢复演练、容量及实体手机验收。安全清理、备份恢复和负载证据分别见 Stage 05D 对应指南。

## 当前检测结果

| 项目 | 结果 |
| --- | --- |
| PDF Worker 监听 | `192.168.1.20:18081` |
| QuickDrop 监听 | `127.0.0.1:18080` |
| 二维码基础地址 | `http://192.168.1.20:18081` |
| 机构局域网候选地址 | `192.168.1.20/24`，网关 `192.168.1.1` |
| Tailscale 地址 | `100.64.0.10`，服务已运行 |
| UFW | 未启用 |
| 当前模式 | B：机构局域网测试模式 |

Tailscale 当前报告 DNS 服务器不可达警告。该警告不影响本阶段的本机隧道访问，但在选择 Tailscale 方案前需要单独排查。

第三阶段验收结束后，经用户明确确认，PDF Worker 已切换到局域网地址用于手机扫码测试。Windows `192.168.1.67/24` 已直接取得测试 PDF；没有修改防火墙、路由、Tailscale Serve 或 ACL。原配置备份为项目外权限受限的 `/home/user/.qr-exercise-env-before-lan-test`。
...
```dotenv
PDF_WORKER_BIND_ADDRESS=192.168.1.20
PUBLIC_QR_BASE_URL=http://192.168.1.20:18081
```

重建服务后，在 Windows 和手机浏览器中分别访问：

```text
http://192.168.1.20:18081/health
http://192.168.1.20:18081/r/一个测试资料编号
```
...
```bash
sudo ufw allow from 192.168.1.0/24 to any port 18081 proto tcp
```
...
当前服务器 Tailscale IP 是 `100.64.0.10`。若要试用，应先解决当前 DNS 警告，评审 ACL，再显式配置监听地址和二维码基础地址。手机也必须安装并登录 Tailscale。
