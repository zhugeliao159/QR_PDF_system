...
当前仍是 `192.168.100.20:18081` 的机构 LAN HTTP 测试模式，未修改防火墙、路由、Tailscale 或 DNS，也未执行公网部署。正式公网前必须完成稳定域名与 HTTPS、可信反向代理、学生/管理员入口隔离、Cookie Secure、告警监控、异机加密备份与恢复演练、容量及实体手机验收。安全清理、备份恢复和负载证据分别见 Stage 05D 对应指南。

## 当前检测结果

| 项目 | 结果 |
| --- | --- |
| PDF Worker 监听 | `192.168.100.20:18081` |
| QuickDrop 监听 | `127.0.0.1:18080` |
| 二维码基础地址 | `http://192.168.100.20:18081` |
...
第三阶段验收结束后，经用户明确确认，PDF Worker 已切换到局域网地址用于手机扫码测试。Windows `192.168.101.67/23` 已直接取得测试 PDF；没有修改防火墙、路由、Tailscale Serve 或 ACL。原配置备份为项目外权限受限的 `/home/user/.qr-exercise-env-before-lan-test`。
