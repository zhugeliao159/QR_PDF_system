...
- 当前部署为纯 LAN：管理端 `192.168.1.20:18081`；学生公网占位入口 `127.0.0.1:18082`。
- Funnel 关闭，状态为 `No serve config`。
- SQLite schema 6，完整性检查通过；不要删除或重建正式 `data/`。
- 最终自动化基线为 `176 passed`，匿名原件泄露审计通过。
- Stage 6 已推送至 `agent/stage6-batch-management`，Draft PR：`<pull-request-url>`；服务器日常工作区仍保持在 `main`。
- 二级删除密码尚未由用户设置，永久删除功能会安全地显示为禁用。

## 主要入口

- 管理后台：`http://192.168.1.20:18081/admin`
